...
for (let i = 0;i < document.querySelectorAll(".bookContainer").length;i++) 
    {
        ...
        fetch(`http://localhost:3000/bookings/del/${data.resultFind[i]._id}`,{
            method: "DELETE",
     ...
     document.querySelector('.purchase').addEventListener('click', function(){
        for (let i = 0;i < document.querySelectorAll(".bookContainer").length;i++) 
{
        fetch(`http://localhost:3000/bookings/update/${data.resultFind[i]._id}`,{
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify()
        })
